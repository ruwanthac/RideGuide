...
export const TowJobHistoryScreen: React.FC = () => {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setJobs(rows.filter((r) => r.type === 'tow' && r.status === 'completed'));
    } catch (e) {
        ...
        <FlatList
          ...
          renderItem={({ item }) => (
            <Card style={styles.card} padded>
              ...
              {item.dropoffAddress ? <Text style={styles.meta}>Drop-off: {item.dropoffAddress}</Text> : null}
